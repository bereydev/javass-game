...
public final class Bits32 {
     ...
     * @return the size of the binary representation of the integer 
     *         (based on the position of the most significant bit)
     */
    ...
    /**
     * Public method that give an integer which binary representation is '1' between
     *               start(include) and size(exclude) and '0' otherwise
     *               
     * @param start
    ...
    public static int pack(int v1, int s1, int v2, int s2) {
        Preconditions.checkArgument(s1 > 0 && s1 < Integer.SIZE && bitsSize(v1) <= s1);
        Preconditions.checkArgument(s2 > 0 && s2 < Integer.SIZE && bitsSize(v2) <= s2);
    ...
    public static int pack(int v1, int s1, int v2, int s2, int v3, int s3) {
        Preconditions.checkArgument(s1 > 0 && s1 < Integer.SIZE && bitsSize(v1) <= s1);
        Preconditions.checkArgument(s2 > 0 && s2 < Integer.SIZE && bitsSize(v2) <= s2);
        Preconditions.checkArgument(s3 > 0 && s3 < Integer.SIZE && bitsSize(v3) <= s3);
    ...
    public static int pack(int v1, int s1, int v2, int s2, int v3, int s3, int v4, int s4, int v5, int s5, int v6, int s6, int v7, int s7) {
        ...
        Preconditions.checkArgument(s2 > 0 && s2 < Integer.SIZE && bitsSize(v2) <= s2);
        Preconditions.checkArgument(s3 > 0 && s3 < Integer.SIZE && bitsSize(v3) <= s3);
        Preconditions.checkArgument(s4 > 0 && s4 < Integer.SIZE && bitsSize(v4) <= s4);
        Preconditions.checkArgument(s5 > 0 && s5 < Integer.SIZE && bitsSize(v5) <= s5);
        Preconditions.checkArgument(s6 > 0 && s6 < Integer.SIZE && bitsSize(v6) <= s6);
        Preconditions.checkArgument(s7 > 0 && s7 < Integer.SIZE && bitsSize(v7) <= s7);
